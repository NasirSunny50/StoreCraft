...
describe("getProducts cursor pagination (DB)", () => {
  ...
  it("walks every product once, with no duplicates across pages", async () => {
    ...
    expect(ids.length).toBe(6); // all seeded active products
  });
  ...
  it("inStock filter excludes zero-stock products", async () => {
    ...
    expect(page.items.every((p) => p.stock > 0)).toBe(true);
    expect(page.items.length).toBe(5); // 6 total minus the out-of-stock one
  });
